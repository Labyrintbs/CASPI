...
class MultiWozEvaluator(object):
    ...
    def run_metrics(self, data, eval_act=True):
        if 'all' in cfg.exp_domains:
            metric_results = []
            metric_result = self._get_metric_results(data, eval_act=eval_act)
    ...
    def validation_metric(self, data, return_rich=False, return_per_dialog=False,soft_acc=False,return_each=False, return_dict=None, turn_num=None, return_contrast=False):
        ...
        if return_per_dialog==True:
            ...
            for row in data:
                dial_id = row['dial_id']
                if dial_id  not in dialog_datas:
        ...
        if return_rich:
            ...
            if return_per_dialog==True:
                ...
                for dia_id in dial_ids:
                    all_bleus.append(all_dialogid_bleus[dia_id])
            ...
            success, match, _, dial_num = context_to_eval_response
    ...
    def _get_metric_results(self, data, domain='all', file_list=None, eval_act=True):
        ...
        if eval_act:
            ...
            act_f1 = avg_act_num = avg_diverse_score =0
        ...
        if dial_num:
            ...
            if cfg.bspn_mode == 'bspn' or cfg.enable_dst:
                logging.info('[DST] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f  act f1: %2.2f'%(jg, slot_acc, slot_f1, act_f1))
                logging.info('[DST] [not eval name slots] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f'%(jg_nn, sac_nn, sf1_nn))
                logging.info('[DST] [not eval book slots] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f'%(jg_nb, sac_nb, sf1_nb))
                logging.info('[DST] [not eval name & book slots] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f'%(jg_nnnb, sac_nnnb, sf1_nnnb))
            if cfg.bspn_mode == 'bsdx':
                logging.info('[BDX] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f  act f1: %2.2f'%(jg_, slot_acc_, slot_f1_, act_f1))
                logging.info('[BDX] [not eval name slots] joint goal:%2.2f  slot acc: %2.2f  slot f1: %2.2f'%(jg_nn_, sac_nn_, sf1_nn_))
            logging.info('[CTR] match: %2.2f  success: %2.2f  bleu: %2.2f'%(match, success, bleu))
            logging.info('[CTR] ' + '; '.join(['%s: %2.2f' %(req,acc) for req, acc in req_slots_acc.items()]))
            logging.info('[DOM] accuracy: single %2.2f / multi: %2.2f (%d)'%(accu_single_dom, accu_multi_dom, multi_dom_num))
            if eval_act and (self.reader.multi_acts_record is not None):
                logging.info('[MA] avg acts num %2.2f  avg slots num: %2.2f '%(avg_act_num, avg_diverse_score))
    ...
    def bleu_metric(self, data, eval_dial_list=None, not_return_log=False):
        ...
        for row in data:
            if eval_dial_list and row['dial_id'] +'.json' not in eval_dial_list:
                continue
            if not not_return_log:
                print(f"model:{row['resp_gen']}")
                print(f"label:{row['resp']}")
            ...
            for turn_num, turn in enumerate(dial):
                ...
                if cfg.enable_dspn:
                    ...
                    if cfg.enable_bspn and not cfg.use_true_bspn_for_ctr_eval and \
                        ...
                        # print(f"constraint_dict:{constraint_dict}")
                        # print(f"user turn:{turn['user']}")
                        # print(f"resp turn:{turn['resp']}")
    ...
    def context_to_response_eval(self, data, eval_dial_list = None, same_eval_as_cambridge=False,return_extra=False,return_for_each_dialogue=False,soft_acc=False,return_dict=None,turn_num=None, return_contrast=False):
        ...
        dial_num, successes, successes_true, matches = 0, 0, 0, 0
        if return_for_each_dialogue:
            if return_dict is None and turn_num == 0:
                each_res = {}
            else:
        ...
        for dial_id in dials:
            if eval_dial_list and dial_id +'.json' not in eval_dial_list:
                continue
            if return_for_each_dialogue and turn_num == 0:
                each_res[dial_id] = {}
            dial = dials[dial_id]
            reqs = {}
            goal = {}
            ...
            success_true = 0
            if return_extra:
                success, success_true, match, stats, counts, true_reqs, pred_reqs = evaluated_generated_dialogue
                all_true_reqs.extend(true_reqs)
                all_pred_reqs.extend(pred_reqs)
            else:
                success, match, stats, counts = evaluated_generated_dialogue
            ...
            successes += success
            successes_true += success_true
            matches += match
            dial_num += 1
            if return_for_each_dialogue:
                each_res[dial_id][turn_num] = {}
        ...
        succ_true_rate = successes_true/( float(dial_num) + 1e-10) * 100
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, real_requestables, counts,
                                                          soft_acc=False, same_eval_as_cambridge=False, return_extra=False):
        ...
        # final eval
        if soft_acc:
            success_true = float(success_true)/len(real_requestables)
        else:
            if success_true >= len(real_requestables):
                success_true = 1
            else:
                success_true = 0
        # -------------------------------------------------|
        ...
        for domain in domains_in_goal:
            ...
            true_label = []
            pred_label = []
            not_provided_requestables = []
            for request in set(real_requestables[domain]):
                if request in provided_requestables[domain]:
                    pred_label.append(request)
                    true_label.append(request)
                    

            other_pred_req = list(set(provided_requestables[domain]) - set(pred_label))
            other_real_req = list(set(real_requestables[domain]) - set(true_label))
            
            i = 0
            for request in other_real_req:
                true_label.append(request)
                if i<len(other_pred_req):
                    pred_label.append(other_pred_req[i])
                    i+=1
                else:
                    pred_label.append('none')
             
            if i<len(other_pred_req):
                for j in range(i,len(other_pred_req)):
                    pred_label.append(other_pred_req[j])
                    true_label.append('none')
        ...
        if return_extra:
            return success, success_true, match, stats, counts, all_true_label, all_pred_label
        #------------------Used only during reward leaning (ends) -----------------------#
        else:
