...
class MultiWozReader(_ReaderBase):
    def __init__(self, vocab=None):
        ...
        if 'L_sto' in other_config['policy_loss']:
            if other_config['fn_Qs_file_path_dp'] and os.path.exists(other_config['fn_Qs_file_path_dp']):
                self.fn_Qs_dp = json.loads(open(other_config['fn_Qs_file_path_dp'],'r').read())
            else:
                print("other_config['fn_Qs_file_path_dp'] doesn't exit:",other_config['fn_Qs_file_path_dp'])
        
        self.fn_Gs_dp = {}
        if other_config['fn_Gs_file_path_dp'] and os.path.exists(other_config['fn_Gs_file_path_dp']):
            self.fn_Gs_dp = json.loads(open(other_config['fn_Gs_file_path_dp'],'r').read())
        else:
            print("other_config['fn_Gs_file_path_dp'] doesn't exit:",other_config['fn_Gs_file_path_dp'])
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial['log']):
            ...
            if self.fn_Gs_dp:
                if fn in self.fn_Gs_dp and str(t['turn_num']) in self.fn_Gs_dp[fn]:
                    enc['G'] = float(self.fn_Gs_dp[fn][str(t['turn_num'])]['G'])
                    enc['gamma_dp'] = float(self.fn_Gs_dp[fn][str(t['turn_num'])]['gamma'])
                else:
                    enc['gamma_dp'] = 0.
                    enc['G'] = 0.
                    
            if self.fn_Qs_dp:
                if fn in self.fn_Qs_dp and str(t['turn_num']) in self.fn_Qs_dp[fn]:
                    enc['Q'] = float(self.fn_Qs_dp[fn][str(t['turn_num'])]['Q'])
                    enc['bhProb'] = float(self.fn_Qs_dp[fn][str(t['turn_num'])]['prob'])
                else:
                    enc['Q'] = 0.
                    enc['bhProb'] = 1.
    ...
    def convert_batch(self, py_batch, py_prev, first_turn=False):
        ...
        if 'act_id' in py_batch:
            inputs['act_id'] = py_batch['act_id']
            
        if 'state_id' in py_batch:
            inputs['state_id'] = py_batch['state_id']
            
        if 'G' in py_batch:
            inputs['G_np'] = np.array(py_batch['G'])
            inputs['G_unk_np'] = inputs['G_np']
            
        if 'Q' in py_batch:
            inputs['Q_np'] = np.array(py_batch['Q'])
            inputs['Q_unk_np'] = inputs['Q_np']
        
        if 'bhProb' in py_batch:
            inputs['bhProb_np'] = np.array(py_batch['bhProb'])#utils.padSeqs(py_batch['bhProb'], truncated=cfg.truncated, trunc_method=trunc_method, max_len_overide=inputs['aspn_unk_np'].shape[-1],dtype='float',value=1.0)
            inputs['bhProb_unk_np'] = inputs['bhProb_np']
        
        inputs['dial_id'] = py_batch['dial_id']
        inputs['turn_num'] = py_batch['turn_num']
