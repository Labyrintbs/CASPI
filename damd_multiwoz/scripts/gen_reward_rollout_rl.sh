...
per_epoch_report_path=${root_path}/data/multi-woz-oppe/reward/rl_20240303_bspn_with_cntfact_and_compare_reward_report_${K}_${metric}_${fold}_ratio_${ratio}_dp.csv
dev_list=${root_path}/data/multi-woz-processed/rewardListFile_${K}_${fold}.json

exp_name=rl_bspn_no_cntfact_reward_K_${K}_fold_${fold}_metric_${metric}_seed_${seed}_CntfactRatio_${ratio}
...
python  ${root_path}/model.py -mode train -cfg seed=$seed cuda_device=$cuda \
	exp_no=rl_cntfact_compare_reward_bspn batch_size=64 multi_acts_training=False \
